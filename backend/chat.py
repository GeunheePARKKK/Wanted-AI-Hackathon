...
[이 도구 사용법]
- 왼쪽 패널: 가구 추가(종류 선택 후 +가구 추가), 동선 추가(폭 입력), 선택 항목 삭제, Properties에서 ID/이름/폭/크기 편집
- 가운데 3D 화면: 클릭=선택(이동 기즈모 표시), 드래그=회전, 휠=줌 / 동선 선택 후 노란 점 드래그=경로 수정, 노란 점 더블클릭=경유점 추가, Shift+클릭=경유점 삭제
...
- zone(구역): 문 개폐 구역, 창문 앞 확보 구역 등 비워둬야 하는 공간
- 동선(walkway): 현관→침대 같은 주요 이동 경로. 폭만큼 비어 있어야 함

[배치 상식]
- 주 동선 폭: 최소 600mm, 권장 900mm / 문 개폐 반경 안에는 가구 금지
- 창문 앞 500mm 비우기 (채광·환기·피난) / 책상 앞 750mm (의자 공간) / 옷장 앞 600mm (문 열기) / 침대 측면 300mm 이상
